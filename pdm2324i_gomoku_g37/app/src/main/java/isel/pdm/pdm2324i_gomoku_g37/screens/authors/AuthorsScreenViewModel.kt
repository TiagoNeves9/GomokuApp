...
class AuthorsScreenViewModel(private val service: GomokuService) : ViewModel() {
    ...
    private val _authorsListFlow: MutableStateFlow<LoadState<List<Author>?>> =
        MutableStateFlow(isel.pdm.pdm2324i_gomoku_g37.domain.idle())
    ...
    fun fetchAuthors() {
        _authorsListFlow.value = isel.pdm.pdm2324i_gomoku_g37.domain.loading()
        viewModelScope.launch {
            val result = runCatching { service.fetchAuthors() }
            _authorsListFlow.value = isel.pdm.pdm2324i_gomoku_g37.domain.loaded(result)
        }
    }
}