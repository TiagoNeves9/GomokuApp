...
import isel.pdm.pdm2324i_gomoku_g37.service.InvalidLogin
import isel.pdm.pdm2324i_gomoku_g37.service.UserAlreadyExists
...
fun ProcessError(dismissError: () -> Unit, cause: Throwable) {
    ...
    val errorDetails: Pair<Int, Int> = when (cause) {
        is EmptyUsername -> Pair(
            R.string.error_username_title,
            R.string.username_is_blank_input_error
        )

        is EmptyPassword -> Pair(
            R.string.error_password_title,
            R.string.password_is_blank_input_error
        )

        is EmptyConfirmPassword -> Pair(
            R.string.error_confirm_password_title,
            R.string.repeat_password_is_blank_input_error
        )

        is UnmatchedPasswords -> Pair(
            R.string.error_confirm_password_title,
