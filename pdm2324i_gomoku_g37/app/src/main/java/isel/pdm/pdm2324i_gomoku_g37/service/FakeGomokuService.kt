...
import isel.pdm.pdm2324i_gomoku_g37.domain.board.createBoard
...
private const val FAKE_SERVICE_API_INFO =
    "The Gomoku application is in version " +
            "$FAKE_SERVICE_APP_VERSION and was made by Group 37 - Class 53D"
...
class FakeGomokuService : GomokuService {
    ...
    override suspend fun joinLobby(token: String, lobby: WaitingLobby): Flow<ReadyLobby> = flow {
        ...
        val l = GomokuLobbies.lobbies.firstOrNull { l ->
...
object GomokuLobbies {
    ...
    fun createLobby(userId: String, rules: Rules): String {
        ...
        val waitingLobby = WaitingLobby(
            lobbyId,
            userId,
            rules.boardDim,
            rules.opening.toOpeningString(),
            rules.variant.toVariantString()
        )
